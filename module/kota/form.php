<?php 
	...
	if($kota_id){
		$querykategori = mysqli_query($connection,"SELECT * FROM kota WHERE kota_id ='$kota_id'");
...
<form action="<?php echo BASE_URL."module/kota/action.php?kota_id=$kota_id";?>" method="POSt">
		<div class="element-form">
			<label>Kota</label>
			<span><input type="text" name="kota" value="<?php echo $kota;?>" class="form-control"/></span>
		</div>
		<div class="element-form">
			<label>Tarif</label>
			<span><input type="text" name="tarif" value="<?php echo $tarif;?>" class="form-control"		/></span>
		</div>
		<div class="element-form">
			<label>Status</label>
			<span>
